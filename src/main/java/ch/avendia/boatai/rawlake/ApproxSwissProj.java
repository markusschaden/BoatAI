...
public class ApproxSwissProj {
	...
	private static double CHtoWGSlat(double y, double x) {
		...
		// Process lat
		double lat = (16.9023892 + (3.238272 * x_aux)) - (0.270978 * Math.pow(y_aux, 2)) - (0.002528 * Math.pow(x_aux, 2)) - (0.0447 * Math.pow(y_aux, 2) * x_aux) - (0.0140 * Math.pow(x_aux, 3));
	...
	private static double CHtoWGSlng(double y, double x) {
		...
		// Process long
		double lng = (2.6779094 + (4.728982 * y_aux) + (0.791484 * y_aux * x_aux) + (0.1306 * y_aux * Math.pow(x_aux, 2))) - (0.0436 * Math.pow(y_aux, 3));
	 ...
	 * @return
	 */
	public static double[] WGS84toLV03(double latitude, double longitude, double ellHeight) {
